...
import { IConfigurationService, ConfigurationServiceConfig, ProfileValidationResult } from "../interfaces/IConfigurationService.js";
...
export class ConfigurationService extends BaseService implements IConfigurationService {
   ...
   */
  async saveProfile(profile: MCFProfile): Promise<void> {
    try {
   ...
   */
  async loadProfile(profileId: string): Promise<MCFProfile | null> {
    try {
      ...
      const profile: MCFProfile = JSON.parse(profileData);
   ...
   */
  async getDefaultProfile(): Promise<MCFProfile> {
    try {
      const defaultProfileId = await this.getDefaultProfileId();
   ...
   */
  async validateProfile(profile: MCFProfile): Promise<ProfileValidationResult> {
    const errors: string[] = [];
   ...
   */
  async createProfile(name: string, environment: CLIEnvironment): Promise<MCFProfile> {
   ...
   */
  async cloneProfile(sourceProfileId: string, newProfileId: string): Promise<MCFProfile> {
    ...
    const clonedProfile: MCFProfile = {
      ...sourceProfile,
      id: newProfileId,
   ...
   */
  private async createDefaultProfile(): Promise<MCFProfile> {
